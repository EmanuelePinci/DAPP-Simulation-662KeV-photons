...
void HistoManager::Book()
{
  ...
  if ( ! fFactoryOn ) {
    ...
    // id = 0
    analysisManager->CreateH1("EAbs","Edep in absorber (MeV)", 400, 0., 0.800*MeV);
    // 2. Definizione dei nomi degli assi (il primo argomento '0' è l'ID dell'istogramma)
    analysisManager->SetH1XAxisTitle(0, "Energy (MeV)");
    analysisManager->SetH1YAxisTitle(0, "Occurrence / bin");
    // id = 1
    analysisManager->CreateH1("EGap","Edep in gap (MeV)", 100, 0., 100*MeV); //<-- Non abbiamo più gap
    // id = 2
    analysisManager->CreateH1("LAbs","trackL in absorber (mm)", 100, 0., 1*m);
    // id = 3
    analysisManager->CreateH1("LGap","trackL in gap (mm)", 100, 0., 50*cm); //<-- Non abbiamo più gap
    ...
    analysisManager->CreateNtupleDColumn("Egap"); // column Id = 1 <-- Non abbiamo più gap
    ...
    analysisManager->CreateNtupleDColumn("Lgap"); // column Id = 1 <-- Non abbiamo più gap
    analysisManager->FinishNtuple();
                              ...
                              G4double trackLAbs, G4double trackLGap)
{
  ...
  analysisManager->FillNtupleDColumn(0, 1, energyGap); //<-- Non abbiamo più gap
  analysisManager->AddNtupleRow(0);
  // Fill 2nd ntuple ( id = 1)
  analysisManager->FillNtupleDColumn(1, 0, trackLAbs);
  analysisManager->FillNtupleDColumn(1, 1, trackLGap); //<-- Non abbiamo più gap
